...
public class PlayerStats {
	...
	public EntityPlayerMP getPlayer(String username) {
		return world.getMinecraftServer().getPlayerList().getPlayerByUsername(username);
	}
	...
	public static PlayerStats getPlayerStatsFromFile(String name) {
		...
		File dir = new File(FMLCommonHandler.instance().getMinecraftServerInstance().getEntityWorld().getSaveHandler().getWorldDirectory() + "\\FlansMod players statistics\\");
	...
	public static void printLeaderboardExp(ICommandSender sender) {
		...
		File dir = new File(sender.getServer().getEntityWorld().getSaveHandler().getWorldDirectory() + "\\FlansMod players statistics\\");
		for (File file : dir.listFiles()) {
			checkFileExists(file);
			try {
				NBTTagCompound tags = CompressedStreamTools.read(new DataInputStream(new FileInputStream(file)));
		...
		for (String name : nameList) {
			switch (counter1) {
				case 0:
					sender.sendMessage(new TextComponentString("\u00a76\u00a7l1. " + name + " - Rank " + getPlayerStatsFromFile(name).rank + "(" + getPlayerStatsFromFile(name).totalExp + " Exp)"));
					break;
				case 1:
					sender.sendMessage(new TextComponentString("\u00a74\u00a7l2. " + name + " - Rank " + getPlayerStatsFromFile(name).rank + "(" + getPlayerStatsFromFile(name).totalExp + " Exp)"));
					break;
				case 2:
					sender.sendMessage(new TextComponentString("\u00a7a\u00a7l3. " + name + " - Rank " + getPlayerStatsFromFile(name).rank + "(" + getPlayerStatsFromFile(name).totalExp + " Exp)"));
					break;
				case 3:
					sender.sendMessage(new TextComponentString("\u00a7l4. " + name + " - Rank " + getPlayerStatsFromFile(name).rank + "(" + getPlayerStatsFromFile(name).totalExp + " Exp)"));
					break;
				case 4:
					sender.sendMessage(new TextComponentString("\u00a7l5. " + name + " - Rank " + getPlayerStatsFromFile(name).rank + "(" + getPlayerStatsFromFile(name).totalExp + " Exp)"));
					break;
				case 5:
					sender.sendMessage(new TextComponentString("\u00a7l6. " + name + " - Rank " + getPlayerStatsFromFile(name).rank + "(" + getPlayerStatsFromFile(name).totalExp + " Exp)"));
					break;
				case 6:
					sender.sendMessage(new TextComponentString("\u00a7l7. " + name + " - Rank " + getPlayerStatsFromFile(name).rank + "(" + getPlayerStatsFromFile(name).totalExp + " Exp)"));
					break;
				case 7:
					sender.sendMessage(new TextComponentString("\u00a7l8. " + name + " - Rank " + getPlayerStatsFromFile(name).rank + "(" + getPlayerStatsFromFile(name).totalExp + " Exp)"));
					break;
				case 8:
					sender.sendMessage(new TextComponentString("\u00a7l9. " + name + " - Rank " + getPlayerStatsFromFile(name).rank + "(" + getPlayerStatsFromFile(name).totalExp + " Exp)"));
					break;
				case 9:
					sender.sendMessage(new TextComponentString("\u00a7l10. " + name + " - Rank " + getPlayerStatsFromFile(name).rank + "(" + getPlayerStatsFromFile(name).totalExp + " Exp)"));
					break;
				case 10:
					sender.sendMessage(new TextComponentString("\u00a7b\u00a7l"+nameList.indexOf(sender.getName())+1+". " + sender.getName() + " - Rank " + getPlayerStatsFromFile(name).rank + "(" + getPlayerStatsFromFile(name).totalExp + " Exp)"));
					break;
			}
			counter1++;
	...
	public static int getPlayerLvl(EntityPlayerMP player){
		PlayerStats stats = getPlayerStatsFromFile(player.getName());
		if(stats!=null){
			return stats.totalExp/1000;
		} else {
			new PlayerStats(player.world,player);
			stats = getPlayerStatsFromFile(player.getName());
		}
		return stats.totalExp/1000;
	}
}
